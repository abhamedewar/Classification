...
model = Network()
model.load_state_dict(torch.load('./model/mnist_model.pth', map_location=torch.device(device)))
...
file_upload_title = '<hr font-size: 30px;>Please upload an image file</hr>'
st.markdown(file_upload_title, unsafe_allow_html=True)

file = st.file_uploader('', type=['png'])

if file is None:
    imagefile = r'./data/0.png'
else:
    imagefile = file
